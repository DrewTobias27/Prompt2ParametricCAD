    ...
    Invoke-ReleaseStep "Running frontend behavior checks" {
        ...
        try {
            foreach ($check in $frontendChecks) {
                ...
                if ($LASTEXITCODE -ne 0) {
                    throw (
                        "Frontend check $check failed with exit code " +
                        "$LASTEXITCODE."
                    )
                }
            }
    ...
    if ($CompileSolidWorksPackage) {
        ...
        try {
            ...
            Invoke-ReleaseStep "Compiling downloaded SolidWorks package runner" {
                & $pythonExe -m pytest -m solidworks_compile -q
            }
        }
        finally {
            if ($null -eq $previousCompileSetting) {
                Remove-Item Env:P2P_RUN_SOLIDWORKS_COMPILE `
