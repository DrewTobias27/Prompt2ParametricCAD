...
def test_package_contains_validated_native_replay_and_local_runner():
    ...
    assert b"The editable part will be saved as" in readme
    assert b"If the build does not finish" in readme
    assert b"Check-SolidWorks-Setup.cmd" in readme
    assert b"SLDPRT.replay.log" in readme
    assert b"will not replace an existing part" in readme
    assert b"terminal" not in readme
    assert b"Editability summary" not in readme
    assert b"What the launcher checks" not in readme
